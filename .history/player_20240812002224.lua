...
function Player:MoveTo(position, playerMovement, skipReplication)
    ...
    if not skipReplication then
        ...
        if not playerMovement or self.movedata.movements >= 100 or (overflowed and positionChanged) or (not positionChanged) then -- Teleportation, desync prevention or overflow
