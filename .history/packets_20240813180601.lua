...
local function multiPlayerWrapper(name, criteria, leaveId)
    return function(connection, id, cr, ...)
        ...
        if not leaveId and connection.player and connection.player.id == id then
            id = -1
        end
...
local function PlayerIdentification(data, connection)
    ...
    return protocol.ServerPackets.ServerIdentification(connection)
end

---@type ClientPackets
local ClientPackets = {
    PlayerIdentification = PlayerIdentification,
    SetBlock = basicServerboundWrapper("SetBlock"),
    PositionAndOrientation = basicServerboundWrapper("PositionAndOrientation"),
    Message = basicServerboundWrapper("Message"),
}
