...
function Player:Spawn(player)
    ...
    packets.ServerPackets.SpawnPlayer(self.id, self.name, position.x, position.y, position.z, position.yaw,
        position.pitch, criteria, player and player.connection or nil)
end
...
function Player:MoveTo(position, playerMovement, skipReplication)
    ...
    if not skipReplication then
        ...
        if not config:getValue("server.useRelativeMovement") or (not playerMovement or self.movements >= 100 or (overflowed and positionChanged)) then -- Teleportation, desync prevention or overflow
            packets.ServerPackets.SetPositionAndOrientation(self.id, self.position.x, self.position.y, self.position.z,
                self.position.yaw, self.position.pitch, criteria, playerMovement)
            self.movements = 0
            return
        elseif orientationChanged and positionChanged then
            packets.ServerPackets.PositionAndOrientationUpdate(self.id, difference.x, difference.y, difference.z,
                position.yaw, position.pitch, criteria)
