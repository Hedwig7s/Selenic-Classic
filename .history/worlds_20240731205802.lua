...
function module:PackWorld(world)
    ...
    if type(world) == "string" then
        world = module.loadedWorlds[world]
    end
    local data = pack.pack(">I4", world.size.x*world.size.z*world.size.y)
    for i = 1, world.size.x*world.size.z*world.size.y do
        data = data .. string.char(world.blocks[i] or module.BLOCK_IDS.AIR)
    end
