...
function World:save()
    ...
    data = data .. string.pack(">I2",self.size.x)..string.pack(">I2",self.size.y)..string.pack(">I2",self.size.z)
    data = data .. string.pack(">I2",self.spawn.x)..string.pack(">I2",self.spawn.y)..string.pack(">I2",self.spawn.z)
    local lastBlock = -1
    local count = 0
    ...
    for i = 1, self.size.x*self.size.z*self.size.y+1 do
        ...
        if block == lastBlock then
            ...
            if count > 0 then
                data = data .. string.pack(">I1",lastBlock) .. string.pack(">I4",count)
            end
            lastBlock = block
            count = 1
        end
...
function World:Pack()
    ...
    local lastPercent = 0
    local blocks = {}
    for i = 1, self.size.x*self.size.z*self.size.y do
        table.insert(blocks, string.char(self.blocks[i] or module.BLOCK_IDS.AIR))
        local percent = math.floor(i/(self.size.x*self.size.z*self.size.y)*100)
...
function module:load(name)
    ...
    local sizeX = string.unpack(">I2",data:sub(1, 2))
    local sizeY = string.unpack(">I2",data:sub(3, 4))
    local sizeZ = string.unpack(">I2",data:sub(5, 6))
    local spawnX = string.unpack(">I2",data:sub(7, 8))
    local spawnY = string.unpack(">I2",data:sub(9, 10))
    local spawnZ = string.unpack(">I2",data:sub(11, 12))
    local blocks = {}
    data = data:sub(13)
    for i = 1, #data, 5 do
