...
local function multiPlayerWrapper(name, criteria, leaveId)
    return function(connection, id, cr, ...)
        ...
        for _, connection in pairs(connections) do
            if ((type(criteria) == "function" or criteria == true) and criteriaFunction(connection)) or not criteria then
                base(connection, unpack(data))
            end
        end
    end
...
local ServerPackets = {
    ...
    SetBlock = multiPlayerWrapper("SetBlock",true),
    SpawnPlayer = multiPlayerWrapper("SpawnPlayer",true),
    SetPositionAndOrientation = multiPlayerWrapper("SetPositionAndOrientation",true),
