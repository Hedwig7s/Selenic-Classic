...
function Player:MoveTo(position, playerMovement, skipReplication)
    ...
    if not skipReplication then
        ...
        elseif orientationChanged and positionChanged then
            packets.ServerPackets.PositionAndOrientationUpdate(self.id, difference.x, difference.y, difference.z, difference.yaw, difference.pitch, criteria)
        elseif orientationChanged then
            packets.ServerPackets.OrientationUpdate(self.id, difference.yaw, difference.pitch, criteria)
