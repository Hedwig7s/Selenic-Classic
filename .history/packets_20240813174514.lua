...
---@param criteria boolean|fun(connection: Connection):boolean @Function to determine if a connection should receive the packet
---@return fun(connection: Connection, ...)
local function multiPlayerWrapper(name, leaveId, criteria)
    return function(connection, id, cr, ...)
        ...
        ---@type any|fun(connection: Connection):boolean
        ...
        table.insert(data, ...)
        if connection then
            base(connection, id, unpack(data))
        end
        for _, connection in pairs(connections) do
            if type(criteria) == "function" or criteria == true then
                if criteriaFunction(connection) then
                    base(connection, id, ...)
                end
            end
        end
...
local ServerPackets = {
    ...
    Message = multiPlayerWrapper("Message", true),
