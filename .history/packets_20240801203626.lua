...
local function serverSetBlock(x, y, z, block)
    for _, connection in pairs(connections) do
        local success, err = pcall(connection.write, string.pack(">B",0x06)..string.pack(">H>H>H>B",x,y,z,block))
        if not success then
...
function clientSetBlock(data) 
    local _, x, y, z, mode, block = string.unpack(">B>H>H>H>B>B",data)
    local world = worlds.loadedWorlds[config:getValue("server.defaultWorld")]
    if mode == 0 then
        world:setBlock(x,y,z,block)
    elseif mode == 1 then
        world:setBlock(x,y,z,worlds.block.AIR)
    end
end
