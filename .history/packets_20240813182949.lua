...
local function multiPlayerWrapper(name, criteria, leaveId)
    ---@type fun(connection: Connection?, id:number, cr: nil|fun(connection: Connection):boolean, ...)
    local wrapped = function(connection, id, cr, ...)
        ...
        for _, connection in pairs(connections) do
            if ((type(criteria) == "function" or criteria == true) and criteriaFunction and criteriaFunction(connection, id)) or not criteria then
                base(connection, (connection.player and connection.player.id == id and not leaveId) and dataProvider(-1, ...) or data)
            end
        end
    end
    return wrapped
...
local function basicServerboundWrapper(name)
    return function(data, connection)
        local protocol = getProtocol(connection)
        return protocol.ClientPackets[name](data, connection)
    end
end
...
local function PlayerIdentification(data, connection)
    ...
    if not protocol then
        pcall(function()
            ...
            protocols[7].ServerPackets.DisconnectPlayer(connection, string.sub("Unsupported protocol version: "..protocolVersion, 1, 64))
        end)
        return false
    end
...
local ClientPackets = {
    ...
    [0x05] = basicServerboundWrapper("SetBlock"),
    [0x08] = basicServerboundWrapper("PositionAndOrientation"),
    [0x0D] = basicServerboundWrapper("Message"),
}
