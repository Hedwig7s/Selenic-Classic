...
function World:setBlock(x, y, z, id)
    ...
    --require("./packets").ServerPackets.SetBlock(x, y, z, id)
end
...
function module:load(name)
    ...
    data = data:sub(13)
    for i = 1, #data, 5 do
        local block = string.unpack("B",data:sub(i, i))
        local count = string.unpack("I4",data:sub(i + 1, i + 4))
        for _ = 1, count do
...
function module:loadOrCreate(name)
    if fs.existsSync("./worlds/"..name..".hworld") then
        ...
        local world = World.new(name, {x = 256, y = 128, z = 256}, {x = 0, y = 64, z = 0})
