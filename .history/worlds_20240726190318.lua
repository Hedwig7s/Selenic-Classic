...
function World:save()
    local data = ""
    data = data .. numberutil:toshort(self.size.x)..numberutil:toshort(self.size.y)..numberutil:toshort(self.size.z)
    data = data .. numberutil:toshort(self.spawn.x)..numberutil:toshort(self.spawn.y)..numberutil:toshort(self.spawn.z)
    local lastBlock = -1
    local count = 0
    for _, v in pairs(self.blocks) do
        if v == lastBlock then
            count = count + 1
        elseif lastBlock ~= -1 then
            if count > 0 then
                data = data .. string.char(lastBlock)..count
            end
            lastBlock = v
            count = 1
        end
...
function World.new(name, sizeX, sizeY, sizeZ, blocks)
...
function module:loadOrCreate(name)
    ...
    for block in string.gmatch(data, "([^,]+)") do
        local id, count = string.match(block, "(%d+):(%d+)")
        for i = 1, count do
            table.insert(blocks, tonumber(id))
        end
    end
    return World.new(name, 256, 256, 256, blocks)
end
